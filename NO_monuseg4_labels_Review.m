...
%%%%%%%%% Folder Results %%%%%%%%
FolderResults = '/mnt/md0/ricardo/NoiseletProject/ResultsNoiselets_Original_Complete_noOverlap_Monuseg_ecDistArtic2/'; %folder Out'
...
for ClassExp = 4%:length(clasesMonu) %realizar experimento por clase
    imageForExperiment={};
...
%  KFold = 10;
 ...
 %%%% Save Results
   ...
   %%% Number of Nuclei
    Resultados_Nucleos = cell(length(ExperimentosNoiselets),length(imageForExperiment)*2);

    Resultados_NucleosMetodo = cell(length(ExperimentosNoiselets),length(imageForExperiment)*2);
...
ThresholdId = areaIn>areaBin*0.40;
if ThresholdId
countN = countN+1;
else
countN = countN+0;
