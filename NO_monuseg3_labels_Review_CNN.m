...
%%%%%%%%% Folder Results %%%%%%%%
FolderResults = '/mnt/md0/ricardo/NoiseletProject/ResultsNoiselets_Original_Complete_noOverlap_Monuseg_ecDistArtic2_reviewer/'; %folder Out'
...
FolderTestImg = '/mnt/md0/Histopathology/Datasets/BreastNoiseletE/Slides/';
ListTestImages = dir([FolderTestImg,'*/*.png']);
FolderMaskBlindTest = ['/mnt/md0/Histopathology/Datasets/BreastNoiseletE/','Masks/'];
mkdir(FolderMaskBlindTest)
FolderMaskGt = '/mnt/md0/Histopathology/Datasets/BreastNoiseletE/GroundT/';
FolderMaskDeepL = '/mnt/md0/Histopathology/Datasets/BreastNoiseletE/MaksHovernet/';
FolderMaskDeepMethod = '/mnt/md0/Histopathology/Datasets/BreastNoiseletE/maskMethod/';
...
%  KFold = 10;
 ...
 %%%% Save Results
    Resultados_acc_SVM = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_SumadoImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_OriginalImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_Only_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    
    %%%% PRECISION DETECTION PER CASE
    Resultados_PrecisionD_SumadoImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_PrecisionD_OriginalImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_PrecisionD_Only_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    %%%% RECALL DETECTION PER CASE    
    Resultados_RecallD_SumadoImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_RecallD_OriginalImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_RecallD_Only_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    %%%% FSCORE DETECTION PER CASE    
    Resultados_FscoreD_SumadoImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_FscoreD_OriginalImg = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_FscoreD_Only_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    
   %%% Number of Nuclei
    Resultados_Nucleos = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);

    Resultados_NucleosMetodo = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
%%%% Results For DeepLearning ArchitectureBaseline
    Resultados_Img_Deep= cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_PrecisionD_Img_Deep = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_RecallD_Img_Deep = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_FscoreD_Img_Deep = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);

    
    Resultados_Img_Deep_method= cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_PrecisionD_Img_Deep_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_RecallD_Img_Deep_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
    Resultados_FscoreD_Img_Deep_method = cell(length(ExperimentosNoiselets),length(ListTestImages)*2);
...
for Expe = 28
        ...
        AccumTest = 0;     
 for OneOut = 1:1%cv.NumTestSets
       TestImg = ListTestImages;
       
       ImgsTraining = imageForExperiment;
...
for Lo = 1:length(TestImg) %%% this for is always 1, a cicle for her would be used to K-fold validation
    ...
    fprintf('AbriendoImagen\n') 
    ImgTestS = TestImg(Lo).name(1:end-4);
    fprintf('Evaluating %s\n',ImgTestS) 
    ImTest = imread([TestImg(Lo).folder,'/',ImgTestS,'.png']);
    fprintf('Mejorando Imagen\n') 
    MaskName = [FolderMaskBlindTest,ImgTestS,'.mat'];
        %% abrir mascara manual para generar etiquetas
tic
idxT = find(ImgTestS=='_');
    if ~exist(MaskName,'file')
        fprintf('generating image \n')
        
    ImGroundTE = imread([FolderMaskGt,'GT_',ImgTestS(1:idxT-1),'/',ImgTestS,'.png']);
    ImGroundTE=bwlabel(ImGroundTE);
...
countN = 0;
OriginalNuclei = unique(ImGroundTE);
for g=1:length(OriginalNuclei)
    gg = OriginalNuclei(g);
NucleiId = ImGroundTE==g;

MulImgBin=ImEnhance(:,:,1)<255.*uint8(NucleiId);
areaIn = sum(MulImgBin(:)>0);
areaBin = sum(NucleiId(:)>0);
ThresholdId = areaIn>areaBin*0.5;
if ThresholdId
countN = countN+1;
else
countN = countN+0;
...
%%%%%%%%%%%%%%%%%%%%%%
    Resultados_Nucleos{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_Nucleos{Expe,AccumTest*2} = length(unique(ImGroundTE));
    
    Resultados_NucleosMetodo{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_NucleosMetodo{Expe,AccumTest*2} = countN;
...
[Precision_M,Recall_M,Fscore_M] = Nuclei_centroidsBased_metric(ImGroundTE, MaskEvaluate);
    ...
    %%%%%% Baseline Deep Learning vs macenko approach
    MaskEvaluate = load([FolderMaskDeepL,'Slide_',ImgTestS(1:idxT-1),'/mat/',ImgTestS,'.mat']);
    BinaryDeepMask=MaskEvaluate.inst_map>0;
    
%%%%%%%%%%%%%%%%%%%%%%%%% METRICS %%%%%%%%%%%
   fprintf('Calculando Metricas Sin Sumar\n') 
   DiceCoeff = dice( double(BinaryDeepMask),double(ImManualMask>0)); 
   % DETECTION METRIC
   Resultados_Img_Deep{Expe,2*AccumTest-1} = ImgTestS;    
   Resultados_Img_Deep{Expe,AccumTest*2} = DiceCoeff;
   %[Precision_S,Recall_S,Fscore_S] = Nuclei_centroidsBased_metric_Hover(ImGroundTE, MaskEvaluate);
   [Precision_S,Recall_S,Fscore_S] = Nuclei_centroidsBased_metric(ImGroundTE, BinaryDeepMask);
       %%%% PRECISION DETECTION PER CASE
    Resultados_PrecisionD_Img_Deep{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_PrecisionD_Img_Deep{Expe,AccumTest*2} = Precision_S(4);
        %%%% RECALL DETECTION PER CASE    
    Resultados_RecallD_Img_Deep{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_RecallD_Img_Deep{Expe,AccumTest*2} = Recall_S(4);
        %%%% FSCORE DETECTION PER CASE    
    Resultados_FscoreD_Img_Deep{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_FscoreD_Img_Deep{Expe,AccumTest*2} = Fscore_S(4);
    ...
    %%%%%% Baseline Deep Learning, this use the deep lerning in the image
    %%%%%% without background
    
    MaskOriginal = load([FolderMaskDeepL,'Slide_',ImgTestS(1:idxT-1),'/mat/',ImgTestS,'.mat']);
    BinaryDeepMask=MaskOriginal.inst_map>0;
    MaskEvaluate =load([FolderMaskDeepMethod,'/mat/',ImgTestS,'.mat'])
    BinaryDeepMaskEvaluate=MaskEvaluate.inst_map>0;
    sumaBinary = or(BinaryDeepMaskEvaluate, BinaryDeepMask);

%%%%%%%%%%%%%%%%%%%%%%%%% METRICS %%%%%%%%%%%
   fprintf('Calculando Metricas Sin Sumar\n') 
   DiceCoeff = dice( double(sumaBinary),double(ImManualMask>0)); 
   % DETECTION METRIC
   Resultados_Img_Deep_method{Expe,2*AccumTest-1} = ImgTestS;    
   Resultados_Img_Deep_method{Expe,AccumTest*2} = DiceCoeff;
   [Precision_S,Recall_S,Fscore_S] = Nuclei_centroidsBased_metric(ImGroundTE, sumaBinary);
       %%%% PRECISION DETECTION PER CASE
    Resultados_PrecisionD_Img_Deep_method{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_PrecisionD_Img_Deep_method{Expe,AccumTest*2} = Precision_S(4);
        %%%% RECALL DETECTION PER CASE    
    Resultados_RecallD_Img_Deep_method{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_RecallD_Img_Deep_method{Expe,AccumTest*2} = Recall_S(4);
        %%%% FSCORE DETECTION PER CASE    
    Resultados_FscoreD_Img_Deep_method{Expe,2*AccumTest-1} = ImgTestS;
    Resultados_FscoreD_Img_Deep_method{Expe,AccumTest*2} = Fscore_S(4);
    ...
    save([FolderResults,'Results_',char(SelClass),'_Space_review.mat'],...,
    ...
    'Resultados_Nucleos',...,
    'Resultados_NucleosMetodo',...,
    'Resultados_Img_Deep',...,
'Resultados_PrecisionD_Img_Deep',...,
'Resultados_RecallD_Img_Deep',...,
'Resultados_FscoreD_Img_Deep',...,
    'Resultados_Img_Deep_method',...,
'Resultados_PrecisionD_Img_Deep_method',...,
'Resultados_RecallD_Img_Deep_method',...,
'Resultados_FscoreD_Img_Deep_method')
